...
#		print(rest)
		...
		prom_restantes = str(round(Decimal(restantes / x), 2))
#		print(prom_restantes)		
		
		label = porc_gan + '   prom rest ' + prom_restantes
...
xy_chart = pygal.XY(stroke=False, dots_size=5, show_y_guides=False, show_legend=False, style=DarkStyle, x_title='Fichas de ataque', y_title='Fichas defensa')
xy_chart.title = 'Probabilidad De Victoria Contiendas Reiteradas TEG.'
#xy_chart.x_labels = map(str, range(1,41))
#xy_chart.y_labels = map(str, range(1,41))
xy_chart.add("A", lista)
xy_chart.render_to_file('teg.svg')
